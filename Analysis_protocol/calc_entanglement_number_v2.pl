...
my ($help, $input_cor, $traj, $start, $end, $mask, $output_dir, $restart, $round_threshold);
GetOptions(
 'help|h!' => \$help,
 'input|i=s' => \$input_cor,
 'traj|t=s' => \$traj,
 ...
 'restart|r=s' => \$restart,
 'round_threshold|c=s' => \$round_threshold,
);
...
my $usage = "
  Usage: perl calc_entanglement_number.pl
              ...
              [--round_threshold | -c] <float number between 0 and 1> as a threshold 
                                         for rounding. Default 0.5.
              [--help | -h]\n\n";
...
if(!defined($round_threshold))
{
  $round_threshold = 0.5;
}
elsif($round_threshold <= 0 | $round_threshold >= 1)
{
  die("ERROR: --round_threshold | -c must between 0 and 1.");
}
...
my %sel_lookup = map { $_ => 1 } @sel_idx;

my @native_cor = parse_cor($input_cor);
my $native_natom = @native_cor;

my @native_contact_list = calc_contact_list(\@native_cor, $cutoff, \%sel_lookup);

my @G_native_list = calc_G_list(\@native_cor, \@native_contact_list, $cutoff, \%sel_lookup);
#print "@G_native_list \n";
...
if($traj =~ /\.cor$/)
{
  ...
  printf DAT ("# Total native contact in reference structure: %d\n", $#native_contact_list+1);
  printf DAT ("%8s %8s %8s %8s %8s %8s %8s\n", "|g|max", "G1", "G2", "G3", "G4", "G5", "fGtot");
  close(DAT);
  ...
  my @G_traj_list = calc_G_list(\@traj_cor, \@native_contact_list, $cutoff, \%sel_lookup);
  ...
  {
    if(($G_traj_list[$i][0] ne 'nan'))
    {
      $num_nc_traj += 1;
      for(my $j = 0; $j < 2; $j++)
      {
        if(($G_native_list[$i][$j] * $G_traj_list[$i][$j]) >= 0)
        {
          if(abs($G_native_list[$i][$j]) < abs($G_traj_list[$i][$j]))
          {
            $G1++;
          }
          elsif(abs($G_native_list[$i][$j]) > abs($G_traj_list[$i][$j]))
          {
            $G3++;
          }
        }
        ...
        if($max_g < abs($G_traj_list[$i][$j]))
        {
          $max_g = abs($G_traj_list[$i][$j]);
        }
      }
    }
  }
  ...
  if($num_nc_traj != 0)
  {
    $fGtot = ($G1+$G2+$G3+$G4+$G5)/$num_nc_traj/2;
  }
  if($max_g == -999)
  {
    $max_g = 0;
  ...
  printf DAT ("%8d %8.1f %8.1f %8.1f %8.1f %8.1f %8.4f\n", $max_g, $G1/2, $G2/2, $G3/2, $G4/2, $G5/2, $fGtot);
  close(DAT);
  exit;
}
...
if($restart && -s "${output_dir}/G_${name}.dat")
{
  ...
  printf DAT_COPY ("# Total native contact in reference structure: %d\n", $#native_contact_list+1);
  printf DAT_COPY ("%8s %8s %8s %8s %8s %8s %8s\n", "|g|max", "G1", "G2", "G3", "G4", "G5", "fGtot");
...
for(my $i = 1; $i <= $nframe; $i++)
{
  ...
  if($i >= $start && $i <= $end)
  {
    ...
    {
      if(($G_traj_list[$j]->[0] ne 'nan'))
      {
        $num_nc_traj += 1;
        for(my $k = 0; $k < 2; $k++)
        {
          if(($G_native_list[$j]->[$k] * $G_traj_list[$j]->[$k]) >= 0)
          {
            if(abs($G_native_list[$j]->[$k]) < abs($G_traj_list[$j]->[$k]))
            {
              $G1++;
            }
            elsif(abs($G_native_list[$j]->[$k]) > abs($G_traj_list[$j]->[$k]))
            {
              $G3++;
            }
          }
          ...
          if($max_g < abs($G_traj_list[$j]->[$k]))
          {
            $max_g = abs($G_traj_list[$j]->[$k]);
          }
        } 
      } 
    }
  ...
  for(my $i = 0; $i < @coor-4; $i++)
  {
    for(my $j = $i + 4; $j < @coor; $j++)
    {
      ...
      if($distance <= $cutoff)
      {
        ...
        elsif($sel_lookup{$i+1} && $sel_lookup{$j+1})
        {
          push@c_map, [$i, $j];
        }
      }
  ...
  for(my $i = 0; $i < @sel; $i++)
  {
    ...
    if($dist <= $cutoff)
    {
      ...
      for(my $k = 0; $k < 2; $k++)
      {
        ...
        $G0[$k] = round_num($G0[$k]/4/3.14, $round_threshold);
      }
...
sub round_num
{
  ...
  if ($n >= 0) {
    return $frac >= $thresh ? $int_part + 1 : $int_part;
  }
  else {
    return $frac <= -$thresh ? $int_part - 1 : $int_part;
  }
