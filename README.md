...
| Analysis_protocol/**get_post_trans_order_parameters.py** | Collect the pre-calculated order parameters $`Q_{\mathrm{act}}`$ and $`G`$ and output .npy data files. ([Learn more](../../wikis/help_wiki/get_post_trans_order_parameters.py)) <br>Scripts needed: `Analysis_protocol/calc_native_contact_fraction.pl` and `Analysis_protocol/calc_entanglement_number.pl` |
